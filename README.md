# 👋 Hey, I'm Codexx

### 💫 About Me
I'm a **SOC Cybersecurity Professional** and **Ethical Hacking Expert** who loves solving complex problems, automating security workflows, and designing intelligent UI/UX systems for real-world projects.  
I believe in creating tools that make systems safer, smarter, and more explainable.

---

### 🧰 Tech Stack

#### 🛡️ Cybersecurity & Cloud
- SIEM (Splunk, QRadar, ELK, Azure Sentinel)
- Threat Hunting & Incident Response
- Vulnerability Assessment & Penetration Testing
- OSINT | SOC Automation | Malware Analysis

#### 💻 Programming & Tools
- Python | JavaScript | Bash | Node.js  
- Git | GitHub Actions | Docker | API Integration  
- Postman | Nmap | Metasploit | Burp Suite  

#### 🎨 UI/UX & Frontend
- React | Next.js | Tailwind CSS | Framer Motion  
- Figma | Adobe XD | Component Design Systems  

#### ⚙️ Data & Analytics
- Power BI | Grafana | MongoDB | SQLite  
- Data Visualization & Dashboard Analytics  

---

### 📊 GitHub Analytics
![CodeXx's GitHub Stats](https://github-readme-stats.vercel.app/api?username=codexx&show_icons=true&theme=radical&count_private=true)
![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=codexx&layout=compact&theme=radical)

---

### 🚀 Featured Projects
- 🔐 **SOC Automation Dashboard** — Real-time SIEM visualization with API integration.  
- 🧠 **Threat Intel Analyzer** — Automated CVE & IOC lookup tool.  
- 🌐 **Hackathon Management System** — Smart UI/UX-based project tracking app.  

---

### 🌱 Currently Exploring
- AI-driven SOC automation  
- Cloud Security (Azure + AWS)  
- Security-Oriented Frontend Systems  

---

### 📫 Connect With Me
- 💼 [LinkedIn](#)
- 🐦 [Twitter](#)
- 🌐 [Portfolio](#)

---

> “Security is not a feature — it’s a mindset.”  
> — *CodeXx*
